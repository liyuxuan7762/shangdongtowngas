...
山东港华燃气 Home Assistant 自定义集成 —— 自动获取燃气表累计用量，对接 HA 能源仪表盘。

> 本集成依赖山东港华燃气微信公众号 H5 接口，使用前请先在微信中关注「港华燃气」公众号并完成账户绑定。
...
2. 填入仓库地址，类别选择 **Integration**：
   ```
   https://github.com/liyuxuan7762/shangdongtowngas
   ```
3. 回到 HACS 首页搜索「ShanDongTownGas」并下载
4. 重启 Home Assistant

### 方式二：手动安装

1. 在 [Releases](https://github.com/liyuxuan7762/shangdongtowngas/releases) 页面下载最新的 `shandongtowngas.zip`
2. 解压后将 `shandongtowngas/` 目录放入 HA 的 `config/custom_components/` 下
3. 重启 Home Assistant
4. 进入 **设置 → 设备与服务 → 添加集成**，搜索「山东港华燃气」
...
```
https://weixin.shandongtowngas.com.cn/?login=true
```
...
```
https://weixin.shandongtowngas.com.cn/user/querySubsList
```
...
本项目为个人业余项目，与山东港华燃气及其关联公司无任何隶属或授权关系。所有接口均基于微信公众号/H5 页面的公开行为自行抓包整理，仅供个人家庭能源监控使用，请勿用于任何商业用途。如官方接口发生变动导致集成失效，本项目不承担任何责任。
